...
### Navigation
Sticky, translucent, gains a hairline border + denser background once scrolled. Desktop links get a gradient underline that grows in via `scaleX()` from the left on hover. The brand mark (`.brand__mark`, nav and footer) renders from `logo-icon.png` — a tight crop to the mark's actual ink bounding box (ratio ~1.10:1) with a small even margin, generated from `logo-square.png`. The hero's large mark (`.hero__mark`) still uses the full padded `logo-square.png` canvas, which reads fine at large size. Two smaller marks were tried and both still read "stretched" before this: `logo-mark.png` (an asymmetric crop) and `logo-square.png` forced into a `22px` square box — neither was a real CSS distortion, but boxing non-square ink into a square container reads as one. `.brand__mark` now sets only `height` (`24px`) with `width: auto`, so the browser follows the asset's real ratio instead of a fixed box — don't reintroduce a fixed `width` + `height` pair on this element. `line-height: 1` on both the mark and the wordmark, `align-items: center` on `.brand` for vertical centering.

### Hero Scene (signature composition, hero-only)
Rotating conic-gradient ring (CSS `@keyframes`, decorative wrapper) behind the real logo mark, both independently mouse-parallaxed and additionally scroll-scrubbed via GSAP (drift/scale/fade as the hero leaves the viewport) — the ring and the mark are separate wrapper elements specifically so CSS's continuous rotation animation and GSAP's scroll-driven transform never fight over the same `transform` property on the same node. The hero originally also carried floating "capability" chips (UI/UX, Motion design, Desarrollo) as glass pills near the mark; they were removed by explicit client request for a cleaner hero — don't reintroduce free-floating label chips here or elsewhere as a decorative device.
...
### Don't:
- **Don't** add a kicker or eyebrow line above any heading, anywhere, in any section — including inside decorative mockups (a small caps label above a bold mock-UI title was tried and cut during this build for reading too close to the banned shape).
- **Don't** use gradient-clip ("gradient text") for emphasis — tried once on the hero's key word, removed after review. Emphasis comes from color, weight, or size, not a text-fill gradient.
- **Don't** use emoji or generic Unicode glyphs as icons — authored inline SVG only, one consistent stroke weight.
- **Don't** stage the hero's depth treatment (rotating ring, dual parallax) anywhere else on the page — it is a one-time composition, not a reusable section background.
- **Don't** introduce a second display typeface — Sora carries every heading and all UI chrome; Inter is body-only.
- **Don't** add free-floating label/capability chips (pills with a dot + short word, positioned absolutely near a hero visual) — tried in the hero, removed by explicit client request as visual clutter. Real content belongs in real copy, not decorative labels orbiting an image.
